-- ============================================================
-- PHARMANILE COMPLETE DATABASE SETUP & RESET SCRIPT
-- ============================================================
...
-- Disable RLS temporarily to avoid conflicts during drop if any
DO $$ 
DECLARE
    r RECORD;
BEGIN
...
-- Drop Tables (Ordered to handle foreign keys)
...
DECLARE
  ...
  v_today_sales numeric := 0; v_today_profit numeric := 0; v_active_sessions int := 0; v_low_stock int := 0; v_expiring_soon int := 0; v_weekly_data json; v_threshold int;
BEGIN
  ...
  SELECT json_agg(row_to_json(d)) INTO v_weekly_data FROM (
    SELECT to_char(day_series, 'Day') AS name, COALESCE(SUM(CASE WHEN o.payment_method <> 'debt' THEN o.total ELSE 0 END), 0) AS sales, COALESCE(SUM(CASE WHEN o.payment_method = 'debt' THEN o.total ELSE 0 END), 0) AS debts
