-- ============================================================
--  PharmaNile Database Optimization Migration V2
--  Run this ONCE in your Supabase SQL Editor
-- ============================================================

-- ─────────────────────────────────────────────
-- 1. MONTHLY SUMMARIES TABLE (Pre-aggregated Analytics)
-- ─────────────────────────────────────────────
...
DROP POLICY IF EXISTS "pharmacy_monthly_summaries_policy" ON monthly_summaries;
...
-- ─────────────────────────────────────────────
-- 2. INVENTORY SNAPSHOTS (Historical Stock Tracking)
-- ─────────────────────────────────────────────
...
DROP POLICY IF EXISTS "pharmacy_inventory_snapshots_policy" ON inventory_snapshots;
CREATE POLICY "pharmacy_inventory_snapshots_policy"
  ON inventory_snapshots FOR ALL
  USING (pharmacy_id = (auth.jwt()->'user_metadata'->>'pharmacy_id')::uuid);

CREATE INDEX IF NOT EXISTS idx_snapshots_pharmacy_product_date
  ON inventory_snapshots (pharmacy_id, product_id, snapshot_date DESC);

-- ─────────────────────────────────────────────
-- 3. PERFORMANCE INDEXES (Hot-Path Optimization)
-- ─────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_orders_pharmacy_payment ON orders (pharmacy_id, payment_method);
CREATE INDEX IF NOT EXISTS idx_orders_customer ON orders (customer_id) WHERE customer_id IS NOT NULL;

-- Order Items
CREATE INDEX IF NOT EXISTS idx_order_items_order_id ON order_items (order_id);
CREATE INDEX IF NOT EXISTS idx_order_items_pharmacy_product ON order_items (pharmacy_id, product_id);
CREATE INDEX IF NOT EXISTS idx_order_items_batch_id ON order_items (batch_id);

-- Batches (FEFO & Stock Lookups)
CREATE INDEX IF NOT EXISTS idx_batches_product_pharmacy_qty ON batches (product_id, pharmacy_id, quantity DESC);
CREATE INDEX IF NOT EXISTS idx_batches_pharmacy_expiry ON batches (pharmacy_id, expiry_date ASC) WHERE quantity > 0;
CREATE INDEX IF NOT EXISTS idx_batches_barcode ON batches (barcode, pharmacy_id);

-- Products (Search Optimization)
CREATE INDEX IF NOT EXISTS idx_products_pharmacy_name ON products (pharmacy_id, name);
-- Enable Trigram for fuzzy search if not already enabled
CREATE EXTENSION IF NOT EXISTS pg_trgm;
CREATE INDEX IF NOT EXISTS idx_products_name_trgm ON products USING gin (name gin_trgm_ops);

-- Audit & Financials
CREATE INDEX IF NOT EXISTS idx_audit_logs_pharmacy_created ON audit_logs (pharmacy_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_financial_transactions_pharmacy_created ON financial_transactions (pharmacy_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_debt_payments_debtor ON debt_payments (debtor_id, payment_date DESC);
CREATE INDEX IF NOT EXISTS idx_sessions_pharmacy_status ON sessions (pharmacy_id, status);

-- Monthly Summaries
CREATE INDEX IF NOT EXISTS idx_monthly_summaries_pharmacy_year_month ON monthly_summaries (pharmacy_id, year DESC, month DESC);

-- ─────────────────────────────────────────────
-- 4. FUNCTION: upsert_monthly_summary
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- 5. TRIGGER: auto-refresh monthly summary
-- ─────────────────────────────────────────────
...
BEGIN
  FOR r IN
    SELECT DISTINCT pharmacy_id, EXTRACT(YEAR FROM created_at)::int AS yr, EXTRACT(MONTH FROM created_at)::int AS mo
    FROM orders
  LOOP
...
-- ─────────────────────────────────────────────
-- 6. RPC: get_dashboard_stats (Single Call)
-- ─────────────────────────────────────────────
...
BEGIN
  ...
  FROM orders
  WHERE pharmacy_id = p_pharmacy_id AND status = 'completed' AND created_at >= v_today_start;
...
-- ─────────────────────────────────────────────
-- 7. RPC: smart_search (Fast & Fuzzy Search)
-- ─────────────────────────────────────────────
...
BEGIN
  ...
  SELECT 
    ...
    p.company_name,
    ts_rank_cd(to_tsvector('simple', p.name), plainto_tsquery('simple', p_query)) as score
  FROM products p
  LEFT JOIN batches b ON p.id = b.product_id AND b.pharmacy_id = p_pharmacy_id AND b.quantity > 0
  WHERE p.pharmacy_id = p_pharmacy_id
    AND (
      p.name ILIKE '%' || p_query || '%' OR
      b.barcode ILIKE '%' || p_query || '%' OR
...
GRANT EXECUTE ON FUNCTION smart_search(uuid, text, int, int) TO authenticated;

-- ─────────────────────────────────────────────
-- 8. RPC: get_critical_alerts (Unified Alerts)
-- ─────────────────────────────────────────────
...
BEGIN
  ...
  -- High Debt Customers (> 5000 EGP example)
...
GRANT EXECUTE ON FUNCTION get_critical_alerts(uuid) TO authenticated;

-- ─────────────────────────────────────────────
-- 9. RPC: fast_checkout (Atomic Transaction)
-- ─────────────────────────────────────────────
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_cart) LOOP
    ...
    IF jsonb_array_length(COALESCE(v_item->'batch_distributions', '[]'::jsonb)) > 0 THEN
      FOR v_dist IN SELECT * FROM jsonb_array_elements(v_item->'batch_distributions') LOOP
        ...
        UPDATE batches SET quantity = quantity - v_deduction
        WHERE id = (v_dist->>'batch_id')::uuid AND pharmacy_id = p_pharmacy_id;

        INSERT INTO order_items (order_id, product_id, batch_id, name, price, quantity, unit, pharmacy_id)
        VALUES (v_order_id, (v_item->>'product_id')::uuid, (v_dist->>'batch_id')::uuid, v_item->>'name', (v_dist->>'price')::numeric, v_deduction, v_item->>'unit', p_pharmacy_id);
    ...
    IF v_remaining > 0 THEN
      FOR v_batch IN SELECT id, quantity FROM batches WHERE product_id = (v_item->>'product_id')::uuid AND pharmacy_id = p_pharmacy_id AND quantity > 0 ORDER BY expiry_date ASC LOOP
        ...
        UPDATE batches SET quantity = quantity - v_deduction WHERE id = v_batch.id AND pharmacy_id = p_pharmacy_id;
        
        INSERT INTO order_items (order_id, product_id, batch_id, name, price, quantity, unit, pharmacy_id)
        VALUES (v_order_id, (v_item->>'product_id')::uuid, v_batch.id, v_item->>'name', (v_item->>'price')::numeric, v_deduction, v_item->>'unit', p_pharmacy_id);
