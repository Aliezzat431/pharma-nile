...
export function POSProductCard({ product, isExpanded, onAddToCart, onToggleBatches }: POSProductCardProps) {
  return (
    <div className="bg-[var(--glass-surface)] border border-[var(--glass-border)] rounded-xl overflow-hidden transition-colors group hover:border-[var(--nile-teal)]/50 flex flex-col">
      <div
        onClick={() => onAddToCart(product, false)}
        className="p-4 flex items-center justify-between cursor-pointer hover:bg-[var(--glass-surface-heavy)] relative"
      >
        <div>
          <h3 className="font-bold text-lg text-white group-hover:text-[var(--nile-teal)] transition-colors">{product.name}</h3>
        ...
        <div className="flex items-center gap-4 text-right">
          <div>
            <span className="text-[var(--royal-gold)] font-bold text-lg font-cairo cursor-crosshair block">
          ...
          <button
            ...
            className={`text-gray-400 transition-transform bg-[#050505]/40 border border-[var(--glass-border)] p-2 rounded-lg hover:text-white ${isExpanded ? 'rotate-180' : ''}`}
      ...
      <AnimatePresence>
        {isExpanded && product.activeBatches && product.activeBatches.length > 0 && (
          <motion.div
            ...
            className="bg-[#050505]/70 border-t border-[var(--glass-border)] p-3 overflow-hidden"
          >
            <h4 className="text-xs text-gray-400 mb-2 font-cairo">التشغيلات المتاحة للاختيار من السلة (Batches)</h4>
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-right">
                <thead className="text-xs text-gray-500 uppercase bg-[var(--glass-surface)] font-cairo">
                  <tr>
                ...
                <tbody className="font-cairo text-gray-300">
                  {product.activeBatches.map((b: any, idx: number) => {
                    ...
                    return (
                      <tr key={b.id} className={`border-b border-[var(--glass-border)] hover:bg-[var(--glass-surface)] ${isClosest ? 'bg-[var(--nile-teal)]/5 text-[var(--nile-teal)]' : ''}`}>
                        ...
                        <td className="px-3 py-2">
                          ...
                          {isClosest && <span className="ml-2 text-[10px] bg-[var(--nile-teal)] text-black px-1.5 py-0.5 rounded-sm">الأقرب انتهاءً</span>}
                        </td>
