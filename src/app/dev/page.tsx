...
const MetricCard = memo(function MetricCard({
 ...
 purple: 'border-purple-500/20 /8',
 green: 'border-emerald-500/20 /8',
 };
 ...
 className={`bg-glass-surface ${border[color] ?? border.teal} border rounded-2xl p-5 flex flex-col gap-3 backdrop-blur-md`}
 >
...
export default function DevDashboard() {
 ...
 const [nextAllowedRefresh, setNextAllowedRefresh] = useState(0); 
 ...
 try { localStorage.setItem(TERMINAL_LS_KEY, JSON.stringify(trimmed)); } catch { }
 }, [terminalHistory]);
 ...
 }, [isRefreshing, nextAllowedRefresh, pharmacyId, supabase, user?.user_metadata?.pharmacy_id]);
 ...
 ' // \\\\ CPU : ' + systemLoad.cpu + '%',
 ' (| | ) DB Latency: ' + systemLoad.latencyMs + 'ms',
 "/'\\_ _/`\\ Role : DEVELOPER_ADMIN (STAMPED)",
 '\\___)=(___/',
 ];
 break;
 ...
 default:
 out = [...out,
 `nile-bash: command not found: "${cmd}"`,
 ...
 <div className="absolute inset-0 bg-glass-surface from-[var(--nile-teal)]/5 /5 pointer-events-none" />

 <div className="flex items-center gap-4 relative z-10 min-w-0">
 <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-glass-surface to-[var(--nile-teal)]
 ...
 bg-glass-surface from-[var(--nile-teal)] 
 ...
 <motion.div
 layoutId="tab-pill"
 className="absolute inset-0 rounded-2xl bg-glass-surface from-[var(--nile-teal)]/10 border-t border-l border-[var(--nile-teal)]/30 pointer-events-none"
 ...
 <div className="flex items-center gap-2">
 <Activity className="w-4.5 h-4.5 text-[var(--nile-teal)] " />
