    ...
    >
      <div className="glass-panel p-8">
        <h2 className="text-xl font-bold mb-6 border-b border-[var(--glass-border)] pb-4 font-cairo flex items-center gap-2">
          <Palette className="w-5 h-5 text-[var(--nile-teal)]" /> دليل اختصارات لوحة المفاتيح
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={i} className="flex gap-4 p-4 rounded-xl border border-[var(--glass-border)] bg-white/[0.02] hover:bg-white/[0.05] transition-all group">
              <div className="flex flex-col gap-1.5 flex-1">
                <h3 className="text-white font-bold font-cairo text-sm group-hover:text-[var(--nile-teal)] transition-colors">{shortcut.label}</h3>
                <p className="text-xs text-gray-500 font-cairo">{shortcut.description}</p>
              </div>
              <div className="flex items-center gap-1">
                {shortcut.keys.map((k, j) => (
                  <div key={j} className="flex items-center gap-1">
                    <kbd className="px-2 py-1 rounded bg-[var(--glass-surface-heavy)] border border-[var(--glass-border)] text-[10px] font-bold font-inter text-[var(--nile-teal)] shadow-lg min-w-[30px] text-center">
                      {k}
                    </kbd>
        ...
        <div className="mt-8 p-6 rounded-2xl bg-gradient-to-br from-[var(--nile-teal)]/5 to-transparent border border-[var(--nile-teal)]/10">
          <h4 className="text-[var(--nile-teal)] font-bold font-cairo mb-2 flex items-center gap-2">
