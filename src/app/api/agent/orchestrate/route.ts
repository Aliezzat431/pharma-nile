...
2. اختر الخطوة التالية الواحدة فقط، ولا تقفز لأكثر من خطوة في آنٍ واحد.
3. إذا احتجت بيانات من صفحة لم تفتحها بعد → استخدم OPEN_PAGE أولاً.
4. بعد فتح صفحة → يجب SCRAPE لقراءة بياناتها.
5. لا تُجب أبداً من الخيال أو حدسك — البيانات الوحيدة المعتمدة هي ما في DOM Snapshots.
6. إذا وجدت البيانات الكافية → أجب بـ ANSWER فوراً ولا تفتح صفحات إضافية.
7. إذا طُلب منك فعل شيء (بحث، تعبئة نموذج، ضغط زر) → افعله بالتسلسل الصحيح.
8. لا تكرر نفس الخطوة مرتين بدون مبرر.

### صيغة الرد (JSON فقط، بدون أي نص خارجه):
{
  "thought": "تفكيرك التفصيلي في ما يجب فعله الآن وليماذا",
  "action": {
    ...
    "answer": "الإجابة النهائية للمستخدم (بالعربية العامية المصرية)",
    "question": "سؤال التوضيح للمستخدم"
  },
  "confidence": 0.9
}`;
}
