...
export default function ContactSection() {
 ...
 <p className="text-[var(--text-muted)] text-xs font-bold mb-1">{item.label}</p>
 <p className={`font-black text-[var(--text-primary)] text-sm group-hover:${item.color} transition-colors`} dir={item.dir}>
 {item.value}
 </p>
 </div>
 ...
 className="w-full py-4 mt-2 rounded-2xl bg-glass-surface hover:brightness-110 text-[var(--text-primary)] font-extrabold text-sm flex items-center justify-center gap-2.5 shadow-xl shadow-emerald-500/20 active:scale-[0.97] transition-all cursor-pointer"
 >
 ...
 <h3 className="text-[var(--text-primary)] font-black text-base">ساعات العمل</h3>
 </div>
 ...
 <span className="text-[var(--text-muted)] text-xs font-semibold">{item.day}</span>
 <span className={`text-xs font-black ${item.hours.includes('مغلق') ? 'text-[var(--text-inactive)]' : 'text-[var(--text-primary)]'}`} dir="ltr">
 {item.hours}
 </span>
 </div>
 ))}
 ...
 <p className="mt-4 text-[11px] text-[var(--text-inactive)] font-semibold leading-relaxed border-t border-[var(--glass-border)] pt-4">
 ...
 <div className="w-8 h-8 rounded-full bg-glass-surface flex items-center justify-center shadow-lg shadow-blue-500/40 border-2 border-white/30 ">
 <MapPin className="w-4 h-4 text-[var(--text-primary)]" />
 </div>
 <div className="mt-2 px-3 py-1 bg-[#0d1425]/90 border border-[var(--glass-border)] rounded-xl text-[10px] text-[var(--text-primary)] font-bold backdrop-blur-sm whitespace-nowrap">
 مصر الجديدة، القاهرة
 </div>
 </div>
