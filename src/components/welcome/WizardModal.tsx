      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md font-cairo">
          <motion.div
            ...
            className="relative w-full max-w-md p-6 overflow-hidden border bg-[#0d1425]/90 border-[var(--glass-border)] glass-panel rounded-3xl text-right shadow-2xl"
            dir="rtl"
          >
            {/* Close Button */}
            <button
              onClick={onClose}
              className="absolute left-4 top-4 p-2 text-[var(--text-muted)] hover:text-[var(--text-primary)] rounded-lg hover:bg-[var(--glass-surface-heavy)] transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            {phase === 'choose-chain' && (
              <div className="pt-2">
                <div className="text-center mb-6">
                  ...
                  <h3 className="text-xl font-bold text-[var(--text-primary)]">اختر سلسلة الصيدليات</h3>
                  <p className="text-[var(--text-muted)] text-xs mt-1">تحديد شبكة أو مالك السلسلة للدخول إليها</p>
                </div>
                ...
                <div className="relative mb-4">
                  <input
                    ...
                    className="w-full bg-black/40 border border-[var(--glass-border)] rounded-xl py-2.5 pr-10 pl-4 text-[var(--text-primary)] focus:outline-none focus:border-cyan-400/50 transition-all text-sm text-right"
                    placeholder="ابحث عن سلسلة الصيدليات..."
                  />
                  <Search className="absolute right-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-inactive)]" />
                </div>
                ...
                <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1 select-none custom-scrollbar">
                  {isLoading ? (
                    <div className="flex flex-col items-center justify-center py-8 gap-2">
                      ...
                      <span className="text-[var(--text-muted)] text-xs">جاري تحميل السلاسل المتوفرة...</span>
                    </div>
                  ) : filteredChains.length === 0 ? (
                    <div className="text-center py-6">
                      <p className="text-[var(--text-inactive)] text-xs">لا توجد سلاسل مطابقة للبحث.</p>
                      <button
                    ...
                    filteredChains.map((c) => (
                      <button
                        ...
                        className="w-full text-right p-3.5 rounded-xl border border-[var(--glass-border)] bg-white/[0.01] hover:bg-white/[0.05] hover:border-cyan-400/40 transition-all duration-200 flex items-center justify-between group"
                      >
                        ...
                        <ChevronRight className="w-4 h-4 text-[var(--text-inactive)] group-hover:translate-x-[-3px] transition-transform" />
                      </button>
                    ))
                  )}
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-[var(--glass-border)] flex items-center justify-between text-xs">
                  <span className="text-[var(--text-muted)]">لإنشاء صيدلية جديدة في النظام؟</span>
                  <button
                    onClick={() => { onClose(); router.push('/auth/register'); }}
            ...
            {phase === 'verify-password' && (
              <div className="pt-2">
                <div className="text-center mb-6">
                  ...
                  <h3 className="text-xl font-bold text-[var(--text-primary)]">رمز حماية السلسلة</h3>
                  <p className="text-[var(--text-muted)] text-xs mt-1">سلسلة: <span className="text-[var(--text-primary)] font-bold">{selectedChainName}</span></p>
                </div>
                ...
                {passwordSuccess ? (
                  ...
                  <form onSubmit={handleVerifyPassword} className="space-y-4">
                    <div className="space-y-1">
                      <label className="text-xs font-medium text-[var(--text-muted)] mr-1 block">كلمة مرور السلسلة</label>
                      <div className="relative">
                        <input
                          type={showPassword ? 'text' : 'password'}
                          value={chainPassword}
                          onChange={(e) => setChainPassword(e.target.value)}
                          className="w-full bg-black/40 border border-[var(--glass-border)] rounded-xl py-2.5 pr-10 pl-10 text-[var(--text-primary)] focus:outline-none focus:border-cyan-400/50 transition-all text-sm text-left"
                        ...
                        <button
                          ...
                          className="absolute inset-y-0 left-0 pl-3 flex items-center text-[var(--text-inactive)] hover:text-cyan-400 transition-colors"
                          tabIndex={-1}
                        >
                          {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                        </button>
                        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                          <Key className="w-4 h-4 text-[var(--text-inactive)]" />
                        </div>
                      </div>
                    </div>
                    ...
                    <button
                      type="submit"
                      disabled={isLoading}
                      className="w-full py-3 bg-gradient-to-r from-blue-600 to-cyan-500 text-[var(--text-primary)] font-bold text-sm rounded-xl hover:scale-[1.01] active:translate-y-[2px] transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                    >
                      {isLoading ? (
                        <>
                ...
                <div className="mt-6 pt-4 border-t border-[var(--glass-border)] flex items-center justify-between">
                  <button
                    ...
                    className="text-[var(--text-muted)] hover:text-[var(--text-primary)] text-xs font-bold flex items-center gap-1 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="text-[var(--text-inactive)] hover:text-cyan-400 text-xs font-bold flex items-center gap-1 transition-colors"
                  >
            ...
            {phase === 'choose-branch' && (
              <div className="pt-2">
                <div className="text-center mb-6">
                  ...
                  <h3 className="text-xl font-bold text-[var(--text-primary)]">اختر الفرع / الصيدلية</h3>
                  <p className="text-[var(--text-muted)] text-xs mt-1">السلسلة: <span className="text-[var(--text-primary)] font-bold">{selectedChainName}</span></p>
                </div>

                <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1 select-none custom-scrollbar">
                  {branches.length === 0 ? (
                    <div className="text-center py-6">
                      <p className="text-[var(--text-inactive)] text-xs">لا يوجد فروع مضافة لهذه السلسلة بعد.</p>
                      <button
                        onClick={() => { onClose(); router.push('/auth/register'); }}
                        className="mt-2 text-cyan-400 text-xs font-bold hover:underline"
                      >
                    ...
                    branches.map((b) => (
                      <button
                        ...
                        className="w-full text-right p-3.5 rounded-xl border border-[var(--glass-border)] bg-white/[0.01] hover:bg-white/[0.05] hover:border-cyan-400/40 transition-all duration-200 flex flex-col gap-1"
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-[var(--text-primary)] text-sm">{b.name}</span>
                          <ChevronRight className="w-4 h-4 text-[var(--text-inactive)] group-hover:translate-x-[-3px] transition-transform" />
                        </div>
                        {b.address && <span className="text-xs text-[var(--text-muted)]">{b.address}</span>}
                      </button>
                    ))
                  )}
                </div>
