...
export default function Navbar({ onOpenWizard }: NavbarProps) {
 ...
 <div className="w-10 h-10 rounded-xl bg-glass-surface flex items-center justify-center shadow-lg relative group">
 <Pill className="w-5.5 h-5.5 text-[var(--text-primary)] group-hover:rotate-12 transition-transform duration-300" />
 <div className="absolute inset-x-0 bottom-0 h-1/2 bg-glass-surface /10 " />
 </div>
 ...
 <button
 onClick={onOpenWizard}
 className="hidden sm:inline-flex items-center justify-center px-5 py-2.5 rounded-xl text-xs font-bold bg-glass-surface hover:brightness-110 text-[var(--text-primary)] shadow-md active:scale-95 transition-all"
 >
 ابدأ التجربة مجاناً
 </button>
 ...
 <button
 onClick={() => { setIsMobileMenuOpen(false); onOpenWizard(); }}
 className="w-full py-3 rounded-xl text-sm font-bold bg-glass-surface text-[var(--text-primary)] text-center shadow-md font-cairo"
 >
