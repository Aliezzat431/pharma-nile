...
export default function Pricing({ onOpenWizard }: PricingProps) {
 ...
 isYearly
 ? 'bg-glass-surface border-cyan-500/40'
 ...
 plan.isPopular
 ? 'bg-glass-surface /60 to-[#0e1627]/90 border-2 border-cyan-400/60 shadow-2xl shadow-cyan-500/10'
 : 'bg-white/[0.02] border border-white/7 hover:border-white/15 hover:bg-white/[0.03]'
 }`}
 >
 {/* Popular badge */}
 {plan.isPopular && (
 <div className="absolute top-0 inset-x-0 h-1 bg-glass-surface " />
 )}
 {plan.isPopular && (
 <span className="absolute top-5 left-4 px-3 py-1.5 bg-glass-surface text-[var(--text-primary)] text-[10px] font-black rounded-xl tracking-wider uppercase select-none flex items-center gap-1.5 shadow-lg">
 ...
 plan.isPopular
 ? 'bg-glass-surface hover:brightness-110 text-[var(--text-primary)] shadow-xl shadow-blue-500/20'
 : plan.monthlyPrice === null
