    ...
    <div className="w-full space-y-3 text-right" dir="rtl">
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <div key={i} className="bg-white/[0.03] border border-[var(--glass-border)] rounded-xl p-3">
            <p className="text-[10px] text-[var(--text-inactive)] font-bold">{s.label}</p>
            <p className={`text-base font-black font-sans mt-0.5 ${s.cls}`}>{s.val}</p>
          </div>
        ))}
      </div>
      <div className="bg-white/[0.02] border border-[var(--glass-border)] rounded-xl p-3 space-y-2">
        <p className="text-[10px] text-[var(--text-muted)] font-bold">مبيعات الأسبوع</p>
      ...
      <div className="bg-white/[0.02] border border-[var(--glass-border)] rounded-xl p-3 space-y-1.5">
        ...
        {['باراسيتامول أطفال — 15 يوم', 'بروفين 600 — 30 يوم'].map((t, i) => (
          <p key={i} className="text-[10px] text-[var(--text-muted)] font-semibold">{t}</p>
        ))}
      </div>
    </div>
  );
...
function POSMockup() {
  return (
    <div className="w-full space-y-3 text-right" dir="rtl">
      <div className="bg-white/[0.02] border border-[var(--glass-border)] rounded-xl p-3 flex items-center gap-2">
        <div className="flex-1 h-8 bg-black/30 rounded-lg border border-[var(--glass-border)] flex items-center pr-3">
          <span className="text-[11px] text-[var(--text-inactive)]">ابحث عن دواء أو امسح باركود...</span>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2">
        {['أوجمنتين 1ج', 'بنادول إكسترا', 'كلاريتين 10م', 'أوتريفين', 'أسبرين', 'جافيسكون'].map((n, i) => (
          <div key={i} className="bg-white/[0.02] border border-[var(--glass-border)] rounded-xl p-2.5 text-center">
            <p className="text-[10px] text-[var(--text-primary)] font-bold">{n}</p>
      ...
      <div className="bg-white/[0.02] border border-[var(--glass-border)] rounded-xl p-3 space-y-2">
        ...
        <div className="pt-2 border-t border-[var(--glass-border)] flex items-center justify-between">
          <span className="text-[var(--text-primary)] font-bold text-xs">الإجمالي</span>
          <span className="text-cyan-400 font-black font-sans text-base">150 ج.م</span>
        </div>
        <div className="w-full py-2 bg-gradient-to-r from-blue-600 to-cyan-500 rounded-xl text-center text-[var(--text-primary)] font-bold text-xs flex items-center justify-center gap-1.5">
  ...
  return (
    <div className="w-full space-y-2 text-right" dir="rtl">
      <div className="flex items-center justify-between">
        <p className="text-[11px] text-[var(--text-muted)] font-bold">المخزون الحالي: <span className="text-[var(--text-primary)] font-black">1,248 صنف</span></p>
      ...
      <div className="bg-[#090e18] border border-[var(--glass-border)] rounded-xl overflow-hidden">
        <div className="grid grid-cols-4 bg-white/[0.02] border-b border-[var(--glass-border)] text-[9px] text-[var(--text-inactive)] font-bold p-2 gap-1">
        ...
        {rows.map((r, i) => (
          <div key={i} className="grid grid-cols-4 text-[10px] p-2 gap-1 border-b border-white/[0.03] hover:bg-white/[0.01] items-center">
            <span className="text-[var(--text-primary)] font-bold truncate">{r.name}</span>
            <span className="text-[var(--text-muted)]">{r.qty}</span>
            <span className="text-[var(--text-inactive)] font-mono">{r.exp}</span>
    ...
    <section className="py-20 font-cairo" dir="rtl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="text-center space-y-3">
          ...
          <h2 className="text-3xl md:text-5xl font-black text-[var(--text-primary)]">شوف بنفسك كيف يعمل النظام</h2>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2.5 px-5 py-3 rounded-2xl transition-all border font-bold text-sm cursor-pointer ${
                  isActive
                    ? 'bg-[var(--nile-teal-glow)] text-[var(--text-primary)] border-cyan-400/40 shadow-lg'
                    : 'text-[var(--text-inactive)] border-[var(--glass-border)] hover:bg-[var(--glass-surface-heavy)] hover:text-gray-300'
                }`}
              >
        ...
        <div className="bg-white/[0.02] border border-white/8 rounded-3xl overflow-hidden shadow-2xl">
          ...
          {/* Browser chrome */}
          <div className="bg-[#080d17] px-6 py-3.5 flex items-center justify-between border-b border-[var(--glass-border)]">
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-red-500/70" />
              <span className="w-3 h-3 rounded-full bg-yellow-500/70" />
              <span className="w-3 h-3 rounded-full bg-green-500/70" />
            </div>
            <div className="bg-black/30 px-12 py-1 rounded-lg text-[var(--text-inactive)] text-[11px] font-mono">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 min-h-[420px]">
            ...
            {/* Left: Description */}
            <div className="p-8 lg:p-12 flex flex-col justify-center space-y-6 border-b lg:border-b-0 lg:border-l border-[var(--glass-border)]">
              <AnimatePresence mode="wait">
                ...
                >
                  <div>
                    ...
                    <h3 className="text-2xl md:text-3xl font-black text-[var(--text-primary)] leading-tight">{step.title}</h3>
                  </div>
                  <p className="text-[var(--text-muted)] text-sm leading-relaxed font-semibold max-w-md">{step.description}</p>
              ...
              <div className="flex items-center gap-4 pt-4">
                <button
                  ...
                  className="w-10 h-10 rounded-full border border-[var(--glass-border)] flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--glass-surface-heavy)] disabled:opacity-20 transition-all cursor-pointer"
                >
                  <ArrowRight className="w-4.5 h-4.5" />
                </button>
                <div className="flex gap-1.5">
                  {TOUR_STEPS.map((_, i) => (
                    <div key={i} className={`h-1.5 rounded-full transition-all ${currentStep === i ? 'w-7 bg-cyan-400' : 'w-2 bg-[var(--nile-teal-glow)]'}`} />
                  ))}
                </div>
                <button
